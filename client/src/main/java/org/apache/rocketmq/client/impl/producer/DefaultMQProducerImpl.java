...
public class DefaultMQProducerImpl implements MQProducerInner {
    ...
    // rocketmq.message.compressLevel
    ...
    public void start(final boolean startFactory) throws MQClientException {
        switch (this.serviceState) {
            case CREATE_JUST:
                ...
                // 同一个jvm进程中，对于同一个group，只能存在一个producer
                ...
                // 添加TBW102 topic路由信息添加到本地缓存中（当前路由信息都是空的）
                // 先创建对象，再填充值
    ...
    private void makeSureStateOK() throws MQClientException {
        // 在start方法正常执行后，serviceState会被置位RUNNING
        // 见start方法
    ...
    private SendResult sendDefaultImpl(
        ...
        // 如果没有获取到该topic的路由信息，则查询topic TBW102的路由信息
        TopicPublishInfo topicPublishInfo = this.tryToFindTopicPublishInfo(msg.getTopic());

        // 下面的逻辑为获取到了topic的路由信息，有可能是该topic的，也有可能是TBW102的
        if (topicPublishInfo != null && topicPublishInfo.ok()) {
            ...
            for (; times < timesTotal; times++) {
                // 第一次的时候，mq为null，第2次为第一次选中的mqSelected，第3次为第2次选中的mqSelected，以此类推。。。
                String lastBrokerName = null == mq ? null : mq.getBrokerName();
                // 优先选择不同broker上的MessageQueue
    ...
    private TopicPublishInfo tryToFindTopicPublishInfo(final String topic) {
        ...
        if (null == topicPublishInfo || !topicPublishInfo.ok()) {
            ...
            // 取名factory，特么其实是一个instance。。。
            // 如果该topic没有路由信息，则不会更新topicPublicInfo数据，haveTopicRouterInfo = false， ok为false
            // 则会进入到下面else的代码逻辑，获取TBW102的路由信息
        ...
        if (topicPublishInfo.isHaveTopicRouterInfo() || topicPublishInfo.ok()) {
            ...
            // isDefault: 是否为默认topic（TBW102，内部自动创建topic使用）
    ...
    private SendResult sendKernelImpl(final Message msg,
        ...
        // 在查找topic路由信息的过程中，会将跟该topic有关的broker信息本地缓存起来
        // 在本地缓存中查找，找不到的话，重新获取topic的路由信息
        ...
        if (brokerAddr != null) {
            // com.rocketmq.sendMessageWithVIPChannel设置
            // 是否使用VIP通道，如果使用VIP通道，则brokerAddr的port会减2（10911 - 2 = 10909）
    ...
    public SendResult send(Message msg, MessageQueue mq, long timeout)
        ...
        long beginStartTime = System.currentTimeMillis();
        this.makeSureStateOK();
        // 校验msg，defaultMQProducer参数的作用是获取msg最大大小（4M）
        // 重复校验了好多次。。。
        Validators.checkMessage(msg, this.defaultMQProducer);
